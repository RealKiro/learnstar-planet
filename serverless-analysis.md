...
> ⚠️ **已归档（2026-07-10 历史分析快照）**：本文是早期"数据库必要性 / 无服务器部署"的一次性分析，内容已严重过期（当时为 Laravel 11 / PHP 8.3、23 张表、~130 端点、单文件 HTML 前端）。项目现已演进为 **Laravel 12 / PHP 8.5、10 系列 125 种宠物、Vue 3 SPA、三端口架构（管理员 / 教师 / 教室端）**，部署以 Docker 多阶段构建为准（详见 `docs/宠物系统全案.md` 与 `docs/系统架构与端口.md`）。本文仅作历史参考，不再更新。

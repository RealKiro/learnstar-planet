# CLAUDE.md — 学宠星球 (LearnStar Planet)
...
21. **入口脚本生成 .env 必须加引号（2026-09-14 修复，严重启动故障）**:
    - **现象**：容器日志 `Failed to parse dotenv file. Encountered unexpected whitespace at [API 机器人]` + `The environment file is invalid!`，随后 `数据库未就绪` 重试 30 次（90 秒）→ 「跳过迁移」→ `php artisan serve` 直接退出，容器被 `restart: unless-stopped` 反复拉起（约 90 秒一轮）。
    - **根因**：`entrypoint.sh` 用 `env | grep -E "^(APP_|...)" > .env` 直出环境变量，`docker-compose.yml` 的 `BOT_NAME=${BOT_NAME:-API 机器人}` 带空格且未加引号 → phpdotenv 解析失败 → Laravel `writeErrorAndDie` 让**所有** artisan 命令退出。**「数据库连不上」是假象**：连 `db:monitor` 都被 .env 拦住，DB 本身没问题。
    - **修复**：入口脚本改为转义 `\ " $ \`` 后整体双引号包裹写出（空格/中文/特殊字符/空值均安全），并新增三条兜底：① 已有 .env 解析失败时自动重建（保留 APP_KEY）；② 生成后自检，失败则打印 artisan 真实报错；③ DB 重试超时时打印末 5 行真实错误（不再只报「未就绪」）。
    - **同类坑**：任何带空格的值（`APP_NAME=学宠 星球`、自定义 `ADMIN_NAME`）都会触发；新增环境变量时**不要**在 compose 默认值里裸写含空格的中文。
    - **顺手加固**：`storage/database.sqlite` 若因历史数据卷被挂成目录，改名保留 `.bak.<时间戳>` 后重建文件（否则 SQLite 永久不可用）。
22. **PHP 8.5 兼容 · 启动日志残留噪音 · 应用时区（2026-09-14 生产日志复盘）**:
    - **PDO 弃用常量**：`config/database.php` 直接引用 `PDO::MYSQL_ATTR_SSL_CA` / `_SSL_VERIFY_SERVER_CERT` / `_MULTI_STATEMENTS`，PHP 8.5 起弃用（PHP 9 将移除），每轮启动刷 4 行 × 6 次 artisan 调用。已改为按运行时可用的常量名动态取（`Pdo\Mysql::ATTR_*` → PHP<8.4 回退旧常量），**新老 PHP 均无告警**。

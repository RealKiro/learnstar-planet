...
class SyncWechatWorkLeave extends Command
{
    ...
    public function handle(WechatWorkAttendanceService $service): int
    {
        ...
        $this->info("同步: {$date}");
        if ($sid = $this->option('school')) {
            $r = $service->syncForSchool((int) $sid, $date);
            $this->info("同步{$r['synced']}跳过{$r['skipped']}");

            return self::SUCCESS;
        }
        $r = $service->syncAll($date);
        $this->info("同步{$r['synced']}跳过{$r['skipped']}" . ($r['failed'] > 0 ? ", {$r['failed']}校失败" : ''));

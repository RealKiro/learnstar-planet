...
class SchoolAdminController extends Controller
{
     ...
     */
    /**
     * 自动生成班级大屏码（4 位数字：年级2位 + 班号2位，如三年级2班 → 0302）
     */
    private function autoGenerateDisplayCode(string $schoolCode, ?string $grade, ?int $classNum, ?ClassRoom $existingClass = null): string
    {
        $name = $existingClass?->name ?? '';
        if (!$classNum && preg_match('/[（(]\s*(\d+)\s*[)）]\s*班/', $name, $m)) {
            $classNum = (int) $m[1];
        }

        $gradeDigit = \App\Services\DisplayCodeService::gradeDigit($grade);
        $classDigit = ($classNum !== null && $classNum > 0)
            ? str_pad((string) $classNum, 2, '0', STR_PAD_LEFT)
            : \App\Services\DisplayCodeService::classNo($name);

        return $gradeDigit . $classDigit;
    }
     ...
     * 查询班级码登录日志（含 IP 地址）
     */
    public function displayLoginLogs(Request $request): JsonResponse
    {
        $school = $request->user()->school;
        $classIds = ClassRoom::where('school_id', $school->id)->pluck('id');
...
'logo_path' => $school->logo_path]]);
    ...
    public function saveAiSettings(Request $request): JsonResponse
    {
        ...
        $validator = \Illuminate\Support\Facades\Validator::make($request->all(), [
            ...
            'providers.*.output_price_per_m' => 'nullable|numeric|min:0',
            'providers.*.currency' => 'nullable|string|max:8',
        ]);
        ...
        // 并合并服务端计数器（tokens_used/total_calls/estimated_cost/balance 取较大值），避免前端覆写清零
        ...
        foreach ($providers as &$p) {
            foreach ($existing as $ep) {
                ...
                // 空 API Key 时从旧配置保留
                if (empty($p['api_key']) && !empty($ep['api_key'])) {
                    $p['api_key'] = $ep['api_key'];
                }
                // 合并计数器：取新旧中更大者，避免前端持旧缓存保存时把累计清零
                foreach (['tokens_used', 'total_calls', 'estimated_cost', 'balance'] as $counter) {
                    if (isset($ep[$counter])) {
                        $p[$counter] = (float) ($p[$counter] ?? 0) > (float) $ep[$counter]
                            ? ($p[$counter] ?? $ep[$counter])
                            : $ep[$counter];
                    }
                }
                // 缺失时补默认币种
                if (empty($p['currency']) && !empty($ep['currency'])) {
                    $p['currency'] = $ep['currency'];
                }
                break;
            }
        }
    ...
    public function getAiUsage(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'data' => [
                ...
                'estimated_cost' => array_sum(array_map(fn ($p) => (float) ($p['estimated_cost'] ?? 0), $byProvider)),

...
class SchoolAdminController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $class = ClassRoom::create([
            ...
            'display_code' => $this->autoGenerateDisplayCode($school, $request->input('grade'), null, $class),
            'status' => 'active',
        ]);
    ...
    public function batchCreateClasses(Request $request): JsonResponse
    {
        ...
        for ($i = 1; $i <= $count; $i++) {
            ...
            $class = ClassRoom::create([
                ...
                'grade' => $grade,
                'year' => $year,
                'display_code' => $this->autoGenerateDisplayCode($school, $grade, $classNum),
                'status' => 'active',
            ]);
     ...
     * 自动生成班级大屏码
     * 格式: {学校码}{年级数字}{班号}
     * 示例: LS301 (学校LS, 三年级, 1班)
     */
    private function autoGenerateDisplayCode(\App\Models\School $school, ?string $grade, ?int $classNum, ?ClassRoom $existingClass = null): string
    {
        $schoolCode = strtoupper(preg_replace('/[^A-Za-z0-9]/', '', substr($school->code ?? 'SCH', 0, 4)));
        if (empty($schoolCode)) {
            $schoolCode = 'SCH';
        }
    ...
    private function generateDisplayCode(ClassRoom $classRoom): string
    {
        ...
        return $this->autoGenerateDisplayCode($school, $grade, (int) $classNo, $classRoom);
    }
}

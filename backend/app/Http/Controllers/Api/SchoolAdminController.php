...
class SchoolAdminController extends Controller
{
    ...
    public function listTeachers(Request $request): JsonResponse
    {
        ...
        $teacherUsers = User::where('school_id', $school->id)
            ...
            ->orderBy('grade_team')->orderBy('nickname')->orderBy('name')
            ->get();
    ...
    public function batchCreateClasses(Request $request): JsonResponse
    {
        ...
        for ($i = 1; $i <= $count; $i++) {
            ...
            $class = ClassRoom::create([
                ...
                'grade' => $grade,
                'year' => $year,
                'display_code' => $this->autoGenerateDisplayCode($school->code ?? 'SCH', $grade, $classNum),
                'status' => 'active',
            ]);
    ...
    {
        $schoolCode = strtoupper(preg_replace('/[^A-Za-z0-9]/', '', substr($schoolCode ?: 'SCH', 0, 4)));
        if (empty($schoolCode)) {
            $schoolCode = 'SCH';
        }

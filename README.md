...
<p align="center">
  ...
  <a href="#-快速开始">🚀 快速开始</a> • <a href="#-部署指南">📦 部署指南</a> • <a href="#-配置说明">⚙️ 配置说明</a> • <a href="#-数据备份与恢复">💾 数据备份与恢复</a><br><br>
  <b>🔎 了解</b>（选型与能力一览）<br>
  <a href="#-功能特性">✨ 功能特性</a> • <a href="#-常见问题faq">❓ 常见问题（FAQ）</a> • <a href="#-技术架构">🏗️ 技术架构</a><br><br>
  <b>🔗 进阶</b>（对接与扩展）<br>
...
> - 全校推广 → [📦 部署指南](#-部署指南)（数据库切换）+ [⚙️ 配置说明](#-配置说明)
...
| **课表管理** | 科目 / 节次 / 单双周排课；教师「我的课表」跨班聚合，改课走审核流（管理员审批后生效）；管理员课表中心支持直接编辑、CSV 批量导入、任课分配与规则排课（单班 / 全校教师冲突约束）；一键导出 CSES 文件，ClassIsland「从 CSES 导入」同步课表 |
...
>
> **默认配置 = 内置 SQLite 数据库，什么都不用装、什么都不用改**，直接进 [▶️ 启动](#-启动)。全校规模（>500 学生）再按下文 [📦 部署指南](#-部署指南) 外接你已有的 MySQL/PostgreSQL 服务器。
...
- 在管理后台「班级列表」创建班级后，系统会自动生成「LS+年级+班号」式班级码（如 LS11），学生在首页输入班级码即可进入教室端；管理员可在「班级列表」一键批量重置班级码前缀

### 🔐 上线前安全检查（**强烈建议，两分钟**）

系统首次启动即可用，**代价是内置了两个默认凭据**。容器启动日志里会自动打印安全告警——如果看到 `🔴 安全告警：检测到默认凭据`，说明还没改。

| 凭据 | 默认值 | 风险 |
|------|--------|------|
| 管理员密码 | `admin123456` | 任何人可登录后台，管理全校数据 |
| API 机器人密码 | `learnstar-bot-2026` | ⚠️ **该账号拥有本校全部班级的查询与操作权限**，可任意加减分 |

**只用于本机试玩** → 可以不改。**一旦接入校园网 / 让学生或老师访问** → 必须改：
...
改完执行 `docker-compose up -d` 重启生效（密码以 `.env` 为唯一真相来源，每次启动同步）。

> 机器人账号若用不上，直接 `BOT_ENABLED=false` 停用即可，比改密码更彻底。
> 机器人账号按设计**无法从管理后台删除**，停用只能走这个开关。
...
### 🗄️ 外置数据库（MySQL / MariaDB / PostgreSQL · 全校规模）
...
企业微信扫码登录 / 通讯录导入：填 `WECHAT_WORK_CORPID` / `WECHAT_WORK_AGENTID` / `WECHAT_WORK_SECRET`（在企业微信管理后台创建自建应用获取）；钉钉/飞书在 `.env` 填 `DINGTALK_APP_KEY` / `DINGTALK_APP_SECRET`、`FEISHU_APP_ID` / `FEISHU_APP_SECRET`。详细步骤见 [🔌 集成与对接](#-集成与对接)。

## 💾 数据备份与恢复

> **⚡ 场景速查**
> - 我要备份数据 → [📤 备份（SQLite 模式）](#-备份sqlite-模式) / [🐬 备份 / 恢复（外置 MySQL 模式）](#-备份--恢复外置-mysql-模式)
> - 我要恢复数据 → [📥 恢复（SQLite 模式）](#-恢复sqlite-模式) / [🐬 备份 / 恢复（外置 MySQL 模式）](#-备份--恢复外置-mysql-模式)
> - 系统坏了想重来 → 先 [📤 备份](#-备份sqlite-模式)，再看 [❓ 常见问题（FAQ）](#-常见问题faq) 的「彻底重置」
...
本项目的四类对外能力都在这一章：REST API 机器人账号（通用对接）、MCP 服务器（AI 机器人插件）、第三方平台登录与同步（企业微信/钉钉/飞书）、ClassIsland 课表同步（CSES）。

| 小节 | 适合谁 |
|--------|--------|
| [REST API 机器人账号（外部系统对接）](#-rest-api-机器人账号外部系统对接) | 想让**自己的项目/脚本**调用积分管理能力 |
| [MCP 服务器（AI 机器人插件）](#-mcp-服务器ai-机器人插件) | 想接 **AstrBot / Claude Desktop** 等 AI 宿主，自然语言管积分 |
| [第三方平台登录与通讯录同步](#-第三方平台登录与通讯录同步) | 学校用**企业微信 / 钉钉 / 飞书**，要扫码登录与名单导入 |
| [ClassIsland 课表同步（CSES）](#-classisland-课表同步cses) | 教室一体机用 **ClassIsland**，想把本系统排的课表导入进去 |

### 🤖 REST API 机器人账号（外部系统对接）
...
| `BOT_USERNAME` / `BOT_PASSWORD` | `api-bot` / `learnstar-bot-2026` | ⚠️ 密码以 .env 为唯一真相来源，每次重启同步，**上线前务必修改**（见 [🔐 上线前安全检查](#-上线前安全检查强烈建议两分钟)） |
| `BOT_NAME` | `API 机器人` | 在教师列表中的显示名 |

安全须知：机器人账号可操作**全部班级**，请只在内网/可信环境使用并修改默认密码（详见 [🔐 上线前安全检查](#-上线前安全检查强烈建议两分钟)）；积分记录的审计字段 `given_by` 会归到该账号；账号无法从管理后台删除（如需停用走 `BOT_ENABLED=false`）。容器每次启动都会检测默认凭据并在日志中打印告警。

### 🧩 MCP 服务器（AI 机器人插件）

[mcp-server/](mcp-server/) 提供标准 MCP 协议服务器（工具：add_score / batch_add_score / query_score / search_student / get_leaderboard / get_dashboard / send_notice），AstrBot、Claude Desktop 等 MCP 宿主配 `LEARNSTAR_API_BASE` + `LEARNSTAR_API_TOKEN` 两个环境变量即可接入——把上文「REST API 机器人账号」的 token 填进去，QQ/微信群即可用自然语言加减分。部署详见 [mcp-server/README.md](mcp-server/README.md)。
...
| **钉钉** | ✅ | `.env`：`DINGTALK_APP_KEY` / `DINGTALK_APP_SECRET` | 扫码登录 + 通讯录导入 |
| **飞书** | ✅ | `.env`：`FEISHU_APP_ID` / `FEISHU_APP_SECRET` | 扫码登录 + 通讯录导入 |
...
### 🏫 ClassIsland 课表同步（CSES）

在本系统完成排课后，可一键导出 **CSES v1** 格式的课表文件，在教室一体机的 [ClassIsland](https://github.com/ClassIsland/ClassIsland) 中通过「从 CSES 导入」直接同步，无需手动重录。

- **教师端**：课表页「导出 CSES」导出**当前班级**课表（含单双周 week_type）；「我的课表」可跨班聚合查看
- **大屏端**：教室大屏页提供免登录导出（`GET /api/v1/display/export-cses`），一体机可配置定时拉取 URL 自动更新
- **管理员**：课表中心可批量导出、直接编辑（即时生效）、CSV 批量导入（支持 dry-run 预览）

> 权限说明：教师的排课修改为**提交申请制**——保存后进入待审，管理员在「课表审批」通过后才生效（防止误改直接进课堂）。管理员编辑与批量导入即时生效。
...
[MIT License](LICENSE) © 2026 RealKiro

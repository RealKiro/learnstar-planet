...
- **AI 助教** — 对接 DeepSeek / OpenAI / 通义千问 / Moonshot，辅助出题与答疑（需自行配置 API Key）
...
| 🔵 | 撤回功能前端按钮 | 后端接口已完成，需在教师端积分记录列表添加撤回按钮 |
| 🔵 | 前端 API 路径对齐 | `api.ts` 中 3 个未使用的端点路径与后端路由不匹配 |
| 🔵 | 年级战场大屏端优化 | 教室端 PK 页面数据展示完善 |

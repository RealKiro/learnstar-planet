...
### 实时广播 ✅
教师发通知（弹窗/横幅/全屏三种），教室大屏通过轮询实时弹出。适合上课喊话、自习课发通知。前后端都已实现。

### 教室大屏 ✅
投屏后显示全班积分之星、最新动态、TOP5排名，加减分时宠物有弹跳/抖动动画反馈。前后端都已实现。

### 课堂加减分 ✅
点击学生头像选行为原因（答对题/作业优秀/违反纪律），可自定义规则和步长，支持批量操作。前后端都已实现。

### 宠物系统 ✅
54种宠物×8大系列×12级进化，积分喂养升级。前后端都已实现，是项目里最完善的功能。

### 跨班PK ✅
同年级各班排行榜、战力分析、发起挑战。前后端都已实现。

### 积分商城 ✅
老师设置奖品规则，学生兑换，老师审核/发货。前后端都已实现。

### 排行榜 ✅
总积分榜/周榜/宠物等级榜。Redis加速支持，没有Redis自动降级到MySQL查询。前后端都已实现。

### 智能考勤 ✅
一键点名，标记出勤/迟到/请假/缺席。前后端都已实现。

### 班级通知 ✅
发通知到班级，家长端可见。前后端都已实现。

### 作业管理 ⚠️
布置作业、生成二维码token、查看提交列表——这些已实现。但**扫码提交的入口路由尚未注册**，目前还不能实际扫码交作业。

### 在线测验 ⚠️
老师端出卷（题库管理、创建测验、开始/停止、查看统计）已实现。但**学生端作答流程还不完整**。

### 成绩管理 ✅
按考试/科目录入成绩（支持批量）、统计分析和分布查看。前后端都已实现。

### 学年升级 ✅
预览升级明细→确认执行事务，自动处理毕业和年级升班。前后端都已实现。

### 学生管理 ✅
Excel批量导入/创建/转班/删除，前后端都已实现。

### 教师管理 ✅
批量创建账号（智能去重用户名）、分配班级、重置密码。前后端都已实现。

### 数据报表 ✅
积分趋势（近4周）、宠物分布、学生进步、分年级/班级统计。前后端都已实现。

### 家长端 ✅
手机浏览器查看孩子积分、宠物、排名、通知。前后端都已实现。

### 第三方登录 ⚠️
后端微信/QQ/人人登录和绑定接口已实现。但**前端登录页的扫码按钮目前只弹提示，还未对接真实OAuth跳转**。后端账号密码登录完全正常。

### 微信小程序 ⚠️
已注册14个页面，目前实际构建了3个（首页、登录、教师看板），其余页面尚无内容。

### SSE实时推送 ⚠️
后端有完整的SSE实现（心跳、超时管理、断线重连协议），但**前端目前使用HTTP轮询代替**，未接入EventSource。

### MCP机器人 ✅
Python标准MCP协议服务器已实现，含7个工具（加分/查分/排行榜/通知/搜索等），可对接AstrBot QQ/微信机器人。API失败时有降级演示数据。

### Excel导出 ❌
Excel导入已实现，但导出接口返回"暂不支持"——尚未实现。

### AI助教 ❌
后端接口返回固定提示"需要配置AI_PROVIDER和AI_API_KEY"，**实际上没有调用任何AI API的代码**。前端有聊天界面但用不了。环境变量定义了但代码未对接。

### 没做的事情
- ✖ 没有手机App（手机浏览器就能访问）
- ✖ 没有视频直播/网课功能
- ✖ 不是商业产品，界面精致度有限，部分页面有演示数据兜底

...
| **作业管理** ⚠️ | 布置作业、生成二维码、查看提交列表已实现，但扫码提交入口路由未注册 |
| **在线测验** ⚠️ | 教师端题库组卷、自动判分已实现，学生端作答流程未完成 |
...
| 功能 | 现状 | 缺失环节 |
|------|------|---------|
| **作业管理（扫码提交）** | 布置作业、二维码 token、提交列表已实现 | 扫码提交入口路由未注册 |
| **在线测验（学生端）** | 教师端组卷、发布、自动判分已实现 | 学生端作答流程未完成 |
| **第三方登录（前端）** | 后端微信/QQ/企业微信/人人绑定接口已实现 | 前端扫码按钮仅弹提示，未跳转 OAuth |
| **微信小程序** | 已注册 14 个页面，3 个核心页（首页/登录/看板）可运行 | 其余 11 个页面无内容 |
| **SSE 实时推送（前端）** | 后端完整 SSE 实现（心跳/超时/断线重连） | 前端用 HTTP 轮询代替 EventSource |
| **Excel 导出** | 导入已实现 | 导出接口返回"暂不支持" |
| **AI 助教** | 前端聊天界面、环境变量已预留 | 后端未对接任何 AI API，实际不可用 |

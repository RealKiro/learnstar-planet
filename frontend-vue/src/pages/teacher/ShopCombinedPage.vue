...
<template>
  <div>
    ...
    <div v-if="activeTab === 'rates'">
      <div class="card shopc-card-mb">
        <div class="shopc-head">
          <h3 class="shopc-title">💱 币种汇率</h3>
          <button class="btn btn-sm btn-primary" @click="showAddRate = !showAddRate">{{ showAddRate ? '取消' : '添加汇率' }}</button>
        </div>
        <p class="shopc-muted-12">
          设定不同币种之间的兑换比例，影响学生在兑换中心的实际兑换结果。
        </p>
        <div v-if="showAddRate" class="shopc-filter">
      ...
      <div v-else-if="rates.length === 0" class="card shopc-empty">暂无汇率规则</div>
      <div v-else class="card shopc-card">
        <div v-for="r in rates" :key="r.id" class="shopc-row">
          <span class="shopc-row-name">{{ r.name }}</span>
          <span class="shopc-secondary">{{ currencyLabels[r.from_currency] || r.from_currency }}</span>
          <span class="shopc-fw-700">→ 1 : {{ r.rate }}</span>
...
.shopc-card-mb { max-width:640px;padding:24px;margin-bottom:16px; }
.shopc-head { display:flex;justify-content:space-between;align-items:center;margin-bottom:16px; }
.shopc-title { font-size:16px;font-weight:600;margin:0; }
.shopc-muted-12 { font-size:12px;color:var(--color-text-secondary);margin-bottom:12px; }
.shopc-filter { display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:12px;align-items:end;margin-bottom:16px;padding:16px;background:var(--color-bg);border-radius:8px; }
.shopc-error { margin:0 0 10px;padding:8px 12px;background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.2);border-radius:8px;color: var(--color-danger-text);font-size:12px; }
.shopc-mb-16 { margin-bottom:16px; }
.shopc-empty { max-width:640px;padding:24px;text-align:center;color:var(--color-text-secondary);font-size:13px; }
.shopc-card { max-width:640px;padding:24px; }
.shopc-row { display:flex;align-items:center;gap:12px;padding:8px 0;border-bottom:1px solid var(--color-border);font-size:13px; }
.shopc-row-name { flex:1;font-weight:500; }
.shopc-secondary { color:var(--color-text-secondary); }
.shopc-fw-700 { font-weight:700; }
</style>

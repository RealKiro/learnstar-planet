      ...
      <div style="background:#1a1a2e;border:1px solid rgba(255,255,255,0.06);border-radius:16px;max-width:420px;width:100%;padding:12px 14px 10px;box-shadow:0 40px 80px rgba(0,0,0,0.8);display:flex;flex-direction:column;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;">
          <div>
            ...
            <div style="display:flex;gap:4px;">
              <div style="flex:1;"><label style="font-size:8px;color:rgba(255,255,255,0.35);display:block;margin-bottom:1px;">年级团队</label><select v-model="createForm.grade_team" class="form-input" style="height:22px;font-size:10px;padding:2px 5px;"><option value="">不指定</option><option v-for="g in grades" :key="g" :value="g + '团队'">{{ g }}团队</option></select></div>
              <div style="flex:1;"><label style="font-size:8px;color:rgba(255,255,255,0.35);display:block;margin-bottom:1px;">科目</label><select v-model="createForm.subject" class="form-input" style="height:22px;font-size:10px;padding:2px 5px;"><option value="">不指定</option><option v-for="s in subjects" :key="s" :value="s">{{ s }}</option></select></div>
            </div>
